...
interface MoveLabelerProps{
  color:boolean,
  piece:[number,number],
  piece_to:[number,number],
  event:number
}
export const EmptyBoard = () :WhitePieceBoard=>{
    const position =
        [
...
export const MoveLabeler = ({color,piece,piece_to}:MoveLabelerProps)=>{
  const file_labels = ['a','b','c','d','e','f','g','h'];
  const piece_file_label = file_labels[color?(piece[1]-1):(8-piece[1])];
  const piece_rank = String(color?(piece[0]):(9-piece[0]));
  const piece_to_file_label = file_labels[color?(piece_to[1]-1):(8-piece_to[1])];
  const piece_to_rank = String(color?(piece_to[0]):(9-piece_to[0]));
  const move = piece_file_label + piece_rank + piece_to_file_label + piece_to_rank;
  return move
}

export const MoveUnLabeler = ({color,move}:{color:boolean,move:string})=>{
  const piece = [move[0].charCodeAt(0)-97,parseInt(move[1])];
  const piece_to= [move[2].charCodeAt(0)-97,parseInt(move[3])];
  const piece_ = [color?piece[0]:9-piece[0],color?piece[1]:9-piece[1]];
  const piece_to_ = [color?piece_to[0]:9-piece_to[0],color?piece_to[1]:9-piece_to[1]];
  return [piece_,piece_to_]
} 