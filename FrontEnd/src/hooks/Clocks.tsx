...
function updateClockTime(mytime:number[],setMytime:SetterOrUpdater<number[]>,id:string,setGameOver:SetterOrUpdater<boolean>,
    setGameOverMessage:SetterOrUpdater<string>) {

    const new_time = mytime.slice(); // Make a copy of the clock_time array
    if (new_time[1] === 0) 
    {
        new_time[0] -= 1;
        new_time[1] = 59;
...
function updateOpClockTime(optime:number[],setOptime:SetterOrUpdater<number[]>,id:string,setGameOver:SetterOrUpdater<boolean>
    ,setGameOverMessage:SetterOrUpdater<string>) {

    const new_time = optime.slice(); // Make a copy of the clock_time array
    if (new_time[1] === 0) 
    {
        new_time[0] -= 1;
        new_time[1] = 59;
